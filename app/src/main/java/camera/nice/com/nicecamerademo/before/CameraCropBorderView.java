...
public class CameraCropBorderView extends View {
	...
	protected void onDraw(Canvas canvas) {
		if (isFirst) {
            ...
            float scaleX = (float)mWidth/(float)screenW;
            float scaleY = (float)mHeight/(float)screenH;
            float scale = Math.max(scaleX, scaleY);
            int currWidth = (int)((float)mWidth / scale);
            int currHeight = (int)((float)mHeight / scale);

			int left = (screenW - currWidth) / 2;
			int top = (screenH - currHeight) / 2;
			int right = left + currWidth;
			int bottom = top + currHeight;
